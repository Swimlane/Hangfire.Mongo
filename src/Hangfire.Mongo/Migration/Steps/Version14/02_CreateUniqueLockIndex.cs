...
namespace Hangfire.Mongo.Migration.Steps.Version14
{
    ...
    {   
        private readonly string _lockResourceFieldName = nameof(DistributedLockDto.Resource);
        ...
        public bool Execute(IMongoDatabase database, MongoStorageOptions storageOptions, IMongoMigrationBag migrationBag)
        {
            ...
            using (var cursor = locksCollection.Indexes.List())
            {
                ...
                foreach (var index in existingResourceIndexes)
                {
                    ...
                    if (indexName.Contains(_lockResourceFieldName))
                    {
            ...
            {
                Name = _lockResourceFieldName,
                Sparse = true,
                Unique = true
            };
            var indexBuilder = Builders<BsonDocument>.IndexKeys;
            var indexModel = new CreateIndexModel<BsonDocument>(indexBuilder.Descending(_lockResourceFieldName), indexOptions);
